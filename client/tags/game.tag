...
this.on('before-mount',() => {
  ...
  if (opts.replay) {
    this.play = uR.db.main.Play.objects.get(this.opts.matches[1])
    this.game = uR.db.main.Game.objects.get(this.play.game)
  } else {
    this.game = uR.db.main.Game.objects.get(this.opts.matches[1])
  }
  this.game.tag = this
})
this.on('mount',() => {
